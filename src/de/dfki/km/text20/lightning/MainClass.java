...
public class MainClass {
	...
	private void init() {
		...
		props.setProperty(UpdateCheck.class, "update.url",
				"http://api.text20.net/common/versioncheck/");
		...
		this.channel = this.pluginManager.getPlugin(Diagnosis.class).channel(
				LightningTracer.class);
		this.internalPluginManager = new InternalPluginManager(
				this.pluginManager);
		...
		FixationWatcher fixationCatcher = new FixationWatcher(
				fixationEvaluator, this.evaluator);
		...
		if (this.dllStatus) {
			...
			// load sounds
			this.soundDing = Applet.newAudioClip(MainClass.class
					.getResource("resources/ding.wav"));
			this.soundError = Applet.newAudioClip(MainClass.class
					.getResource("resources/error.wav"));
	...
	public void setupStatistics() {
		...
		this.addToStatistic("dimension", ""
				+ this.getProperties().getDimension());
		this.addToStatistic("recalibration", ""
				+ this.getProperties().isRecalibration());
		this.addToStatistic("action hotkey", ""
				+ this.getProperties().getActionHotkey());
		this.addToStatistic("status hotkey", ""
				+ this.getProperties().getStatusHotkey());
		this.addToStatistic("use warp", "" + this.getProperties().isUseWarp());
		this.addToStatistic("sound activated", ""
				+ this.getProperties().isSoundActivated());
	...
	public void toggleMode() {
		if (this.isNormalMode) {
			...
			this.addToStatistic("screen brighness",
					this.getEvaluationSettings()[1]);
			this.addToStatistic("setting brightness",
					this.getEvaluationSettings()[2]);
	...
	public void playDing() {
		if (!this.properties.isSoundActivated())
			return;
		this.soundDing.play();
	}
	...
	private boolean checkDll() {
		...
		// create target file
		File destination = new File(System.getenv("SYSTEMROOT")
				+ "/System32/JIntellitype.dll");
		...
		if (!destination.exists()) {
			...
			$(MainClass.class.getResourceAsStream("resources/JIntellitype.zip"))
					.zipstream().unzip(
							System.getenv("SYSTEMROOT") + "/System32/");
			if (destination.exists()) {
				System.out
						.println("... but we copied it to the system directory..\r\n");
				// return successful
				return true;
			}
			...
			String msg = new String(
					"Initializing failed. The DLL 'JIntellitype.dll' could not be copied to the windows/system32 directory. Please copy it yourself as admin.");
