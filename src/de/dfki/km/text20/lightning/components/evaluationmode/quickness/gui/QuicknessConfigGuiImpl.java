...
public class QuicknessConfigGuiImpl extends QuicknessConfigGui implements
		ActionListener {
	...
	public QuicknessConfigGuiImpl(QuicknessMode quicknessMode) {
		...
		// preselect values
		for (int i = 0; i < MainClass.getInstance().getInternalPluginManager()
				.getSaliencyDetectors().size(); i++)
			if (MainClass.getInstance().getInternalPluginManager()
					.getSaliencyDetectors().get(i).getInformation()
					.getDisplayName().equals("Text Detector"))
				this.comboBoxDetector.setSelectedIndex(i);
	...
	private void buttonOKActionPerformed() {
		...
		for (File file : this.files) {
			...
			if (parser.isValid(file)) {
				...
				if (!this.currentFile.equals("")) {
					...
					// add data
					this.quicknessMode.addToData(this.currentFile,
							this.currentWords);
		...
		// set other stuff
		this.quicknessMode.setStartWithDetector(this.checkBoxStartWithDetector
				.isSelected());
		this.quicknessMode.setSteps(Integer.parseInt(this.spinnerSteps
				.getValue().toString()));
		MainClass
				.getInstance()
				.getInternalPluginManager()
				.setCurrentSaliencyDetector(
						((PluginInformation) this.comboBoxDetector
	...
	private void comboBoxDetectorActionPerformed() {
		// this.buttonDetectorConfig.setText(((PluginInformation)
		// this.comboBoxDetector.getSelectedItem()).getDisplayName() +
		// " Configuration");
		this.buttonDetector
				.setEnabled(((PluginInformation) this.comboBoxDetector
						.getSelectedItem()).isGuiAvailable());
	}
	...
	private void buttonDetectorActionPerformed() {
		this.child = MainClass
				.getInstance()
				.getInternalPluginManager()
				.getSaliencyDetectors()
				.get(((PluginInformation) this.comboBoxDetector
						.getSelectedItem()).getId()).getGui();
	...
	private void manageComboBoxSaliencyDetector() {
		...
		for (int i = 0; i < MainClass.getInstance().getInternalPluginManager()
				.getSaliencyDetectors().size(); i++) {
			this.comboBoxDetector.addItem(MainClass.getInstance()
					.getInternalPluginManager().getSaliencyDetectors().get(i)
					.getInformation());
		}

		// preselect the current one
		if (MainClass.getInstance().getInternalPluginManager()
				.getCurrentSaliencyDetector() != null)
			this.comboBoxDetector.setSelectedItem(MainClass.getInstance()
					.getInternalPluginManager().getCurrentSaliencyDetector()
					.getInformation());
		...
		// initialize button
		// this.buttonDetectorConfig.setText(this.internalPluginManager.getCurrentSaliencyDetector().getInformation().getDisplayName()
		// + " Configuration");
		this.buttonDetector.setEnabled(MainClass.getInstance()
				.getInternalPluginManager().getCurrentSaliencyDetector()
				.getInformation().isGuiAvailable());
	}
