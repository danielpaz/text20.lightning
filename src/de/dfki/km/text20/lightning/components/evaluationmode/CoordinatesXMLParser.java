...
public class CoordinatesXMLParser {
    ...
    private boolean handleCharacters(String value) {
        try {
            ...
            if (this.y) {
                ...
                if (new File(this.fileName.replace("PreparedCoordinates.xml", "Image" + this.numberTmp + ".png")).exists()) {
                    this.data.add(new Point(this.xTmp, this.yTmp));
                } else {
                    System.out.println("File " + this.fileName.replace("PreparedCoordinates.xml", "Image" + this.numberTmp + ".png") + " not found.");
                }
    ...
    public boolean isValid(File xmlFile) {
        // create xsd file
        File xsdFile = new File(xmlFile.getAbsolutePath().substring(0, xmlFile.getAbsolutePath().lastIndexOf(File.separator) + 1) + "PreparedCoordinatesPattern.xsd");
        if (!xsdFile.exists()) {
